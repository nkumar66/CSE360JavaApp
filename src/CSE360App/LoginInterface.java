...
/**
 * <p> LoginInterface Class </p>
 * 
 * <p> Description: The Java/FX-based login interface sets up the login page for users, where they can either login with their username & password or enter an invite code.</p>
 */
public class LoginInterface {
    ...
    public LoginInterface(Stage primaryStage) {
        ...
        // Login button action 
        /*
         * This is where the login button logic will go: 
         * if the username and password field are filled, check database and allow user in if they match
         * Else if invite code is filled, check code and send user to sign up page after that.
         */
        loginButton.setOnAction(e -> {
        	/*
        	 *  As is: Checks if the invite code field is empty
        	 *  NEEDS: Method that handles checking if code is valid generated
        	 */
            if (!text_InviteCode.getText().isEmpty()) {
                proceedToSignUp(primaryStage);
                
              /*
               * As is: if the invite code is empty, checks if username and password are valid.  
               * 		If the username and password are valid according to method, sends to main app.
               * NEEDS: More validation techniques
               */
        ...
        layout.getChildren().addAll(label_ApplicationTitle, label_Username, text_UserName, 
                                    label_Password, text_Password, label_InviteCode, text_InviteCode, 
     ...
     * As is: Just sends to a blank scene that says they are logged in
    ...
    // ** ONCE BUILT, SEND USER TO SIGNUP GUI, WHICH WILL SEND THEM TO APP GUI AFTER THEY COMPLETE THAT**
    
    /***
     * ProceedToSignUp: Sends users who enter valid invite code to signup page
     * As is: Sends as long as invite code is filled
     * NEEDS: Create validateInviteCode, then if that returns true use this function
     * @param primaryStage
     */
     ...
     * NEEDS: Need Database up to create this function
     * 
     * 
     * @param inviteCode
     * @return
